...
class UsersController extends AbstractController
{
     ...
     * @param User $user
     * @param Request $request
     * @param ConfirmConsole\Handler $handler
     * @return Response
     */
    public function confirm(User $user, Request $request, ConfirmConsole\Handler $handler): Response
    {
        if (!$this->isCsrfTokenValid('confirm', $request->request->get('token'))) {
            return $this->redirectToRoute('users.show', ['id' => $user->getId()]);
        }

        $command = new ConfirmConsole\Command($user->getId()->getValue());
